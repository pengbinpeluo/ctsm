module AgSysCropTypeWheat
  ...
  contains
     ...
     procedure :: vern_eff          => vern_eff_wheat
     procedure :: photop_eff        => photop_eff_wheat
...
contains
  subroutine vernalization_wheat(this, env, cumvd)
    ...
    dlt_cumvd=this%wheat_vern_days(env, cumvd)
  ...
  function vern_days_wheat(this, env, cumvd) result(dlt_cumvd)
    ...
    real(r8) :: vd, vd1, vd2
    dlt_cumvd=0._r8
    if ((env%tmin < 15.0_r8) .and. (env%tmax > 0.0_r8)) then
      !!Cold
      vd1 = 1.4_r8 - 0.0778_r8 * env%tc
      vd2 = 0.5_r8 + 13.44_r8 / ((env%tmax-env%tmin + 3._r8)**2) * env%tc
      vd = min (vd1, vd2)
      dlt_cumvd = max (vd, 0.0)
    end if
    if ((env%tmax > 30._r8) .and. ((cumvd + dlt_cumvd) < 10._r8)) then
      !!high temperature will reduce vernalization
      dlt_cumvd = - 0.5_r8*(env%tmax - 30.0_r8)
  ...
  function get_stress_phenol_emerging_phase_wheat(this, env, cumvd, sw_avail_ratio)
    class(agsys_crop_type_wheat), intent(in) :: this
    type(agsys_env_inputs_type),  intent(in) :: env
    real(r8), intent(in) :: cumvd
    ...
    stress_phenol=min(this%vern_effect(cumvd), this%photop_eff(env))*interpolation(sw_avail_ratio, this%rc_sw_emerg_rate)
